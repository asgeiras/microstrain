...
if($list == '') { echo "You did not select any strain<br>"; }
else {
...
else { echo " <span style='font-weight: bold; color: blue;'>", $n, "</span> ";} 
}
...
<table class='sample' border='1'><br>

<tr> <th>Strain</th> <th width='300px'>Genotype</th> <th>Recipient</th> <th>Donor</th> <th width='300px'>Comment</th> <th>Signature</th> <th>Created</th> <th>Select<input type='hidden' name='toggle' value='set' />
<input type='button' name='CheckAll' value='All' onClick='checkAll(document.selectRow)'>
<input type='button' name='UnCheckAll' value='None' onClick='uncheckAll(document.selectRow)'>
</th></tr><form action='index.php?mode=myList' name='selectRow' method='post'><input type='submit' title='Show a table of only the selected strains' name='show' value='Show selected'/>
<?php if($_SESSION['Usertype'] == 'Superuser')
{
?>
<input type='submit' title='Edit the selected strains' name='edit' value='Edit selected'/>
<?php } ?>
<input type='submit' title='Open the selected strains in a printer-friendly table' name='print' value='print selected'/>
<?php
 ...
 if(($_SESSION['comment'] == 'comment')){
	...
	$comment = preg_replace('/'.preg_quote('¦', '/').'/i', "</span>", $comment);
       ...
       echo "</td><td>"; 
          ...
          else {echo $row['Created'];
          $csv_output .= $row['Created'] . "\n";
          }

       echo "</td><td><div align=center><input type=checkbox name='selected[]' value=". $row['Strain']. "></div></td></tr>";
...
else {echo "<span style='color: red'><strong>You need to provide at least one keyword!</strong></span><br>";}
?>
</body>  
</html> 
